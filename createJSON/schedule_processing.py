...
def process_schedule_response(response_schedule, semester, institute=None, speciality=None, group=None, teacher=None):
    """
    Обрабатывает ответ о расписании и сохраняет данные в JSON.
    Args:
        response_schedule: HTTP-ответ.
        semester (int): Номер семестра.
        institute (str, optional): Институт. Defaults to None.
        speciality (str, optional): Специальность. Defaults to None.
        group (str, optional): Группа. Defaults to None.
        teacher (str, optional): Преподаватель. Defaults to None.

    Returns:
        tuple: Кортеж с JSON-строкой и путем к JSON-файлу.
    """
    ...
    if semester in semester_id_mapping and semester not in [1, 2, 8]:
        if response_schedule.status_code == 200:
            response_json, output_json_file = process_schedule_response_mapped(response_schedule, semester, teacher, institute, speciality, group, semester_id_mapping)
        else:
            print(
        ...
        if response_schedule.status_code == 200:
            response_json, output_json_file = process_schedule_response_unmapped(response_schedule, semester, institute, speciality, group, teacher, semester_id_mapping)
        else:
            print(
                f"Ошибка запроса для расписания. Код статуса: {response_schedule.status_code}")
    return response_json, output_json_file

def process_schedule_response_mapped(response_schedule, semester, teacher, institute, speciality, group, semester_id_mapping):
    """
    Метод для обработки запроса, когда семестр входит в маппинг, но это весенний или осенний семестр.
        ...
        Args:
            ...
            speciality (str): Название специальности.
            group (str): Название группы.
            semester_id_mapping (dict): Маппинг номеров семестров.

        Returns:
            tuple: Кортеж из обработанных данных в формате JSON и пути к файлу JSON.
        """
    try:
...
def process_schedule_response_unmapped(response_schedule, semester, institute, speciality, group, teacher, semester_id_mapping):
    """
    Метод для обработки запроса, когда семестр входит в маппинг, но остальные семестры.
    Args:
        response_schedule: HTTP-ответ.
        semester (int): Номер семестра.
        institute (str): Название института.
